...
$base=connect();
$requete = "SELECT * FROM leveldata";
$result = $base->query($requete);
$array = arrayResult($result, 1);
arrayToCsvFile($array, 'drumy.csv');
error_reporting(E_ERROR);
header('Content-Type: application/octet-stream;');
header("Content-Disposition: attachment; filename=drumy" . date('y.m.d-H:i:s') . ".csv;");
header('Content-Length: '.filesize("drumy.csv").';');
readfile("drumy.csv");
